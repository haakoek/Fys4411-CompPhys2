...
void Sampler::sample(bool acceptedStep) {
    ...
    //In the case of quantum dot with 2 electrons, compute mean distance
    double x1 = m_system->getParticles()[0]->getPosition()[0];
    double y1 = m_system->getParticles()[0]->getPosition()[1];
    double x2 = m_system->getParticles()[1]->getPosition()[0];
    double y2 = m_system->getParticles()[1]->getPosition()[1];

    m_meanDistance += sqrt((x2-x1)*(x2-x1) + (y2-y1)*(y2-y1));
    */

    //Sample mean kinetic and mean potential energy.
    ...
    if(m_system->getOptimize()) {

        double dpsi_dalpha = m_system->getWaveFunction()->computeAlphaDerivative(m_system->getParticles());
        double dpsi_dbeta  = m_system->getWaveFunction()->computeBetaDerivative(m_system->getParticles());
...
void Sampler::computeAverages() {
    ...
    MPI_Reduce(&m_meanDistance,&reducedDistance,1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    */
    ...
    /*
         ...
         m_meanDistance        = reducedDistance/(double)m_system->getSize();
    */
    //}
